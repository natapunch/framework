...
namespace Punchenko\Framework\DI;

use Punchenko\Framework\Exception\DInjectorNotFoundClassException;
use Punchenko\Framework\Config\Config;
use Punchenko\Framework\Exception\DInjectorNotFoundServiceException;
...
class DInjector
{
    ...
    public static function get(string $serviceName)
    {
        if (!isset(self::$container[$serviceName])) {
            self::set($serviceName, self::make($serviceName));
        }
    ...
    public static function setConfig(Config $cfg){
        self::$config = $cfg;
        self::$container = $cfg->get('services', []);
    }
    /**
    ...
    public static function make($className, $params = []){
        ...
        if(class_exists($className)){
            try{
                ...
                $params = array_merge(self::lookupConfigParams(self::getClassSlug($className)), $params);
                $paramset = self::resolveParams($constructor_params, $params);
                if(self::isSingletonPattern($reflection_class)){
                    $instance = call_user_func_array([$className, 'getInstance'], $paramset);
     ...
     * @return string
     */
    private static function getClassSlug($className): string {
        $buffer = explode('\\', $className);
